...
def saveMoney():
    ...
    if account in bank:
        ...
        while True:
            if save.isdigit():
                save = int(save)+bank[account]["money"]
                print("存款成功，您的余额为：",save)
                break
            else:
...
def getMoney():
    ...
    if account in bank:
        ...
        if get <=bank[account]["money"]:
            get=bank[account]["money"]-get
            print("取钱成功，您的余额为：",get)
        else:
            print("您的钱不够！")
    else:
        print("您的账号不存在！")
        ...
        while True:
            if transfermoney.isdigit():
                ...
                if transfermoney<=bank[account]["money"]:
                    transfermoney=bank[account]["money"]-transfermoney
                    transfermoney1=bank[account1]["money"]+transfermoney
                    print("转账成功！您的余额为：",transfermoney,"对方账户余额为：",transfermoney1)
                    break
                else:
                    print("您的余额不足！")
